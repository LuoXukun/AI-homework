...
class VGG19(object):
    ...
    def load_model(self):
        ...
        for idx in range(self.net.size()):
            ...
            if 'fc' in self.net.getLayerName(idx):
                ...
                #print weight.shape #[7, 7, 512, 4096]
                # [height, width, in_channel, out_shape] -> [out_shape, height, width, in_channel]
                # [input_shape, output_shape] -> [output_shape, input_shape] 
                weight = weight.transpose(3, 0, 1, 2).flatten().astype(np.float)
                #weight = weight.reshape(-1).astype(np.float)
                bias = bias.reshape(-1).astype(np.float)
                self.net.loadParams(idx, weight, bias, self.filter_quant_params[count])
                count += 1
    ...
    def load_image(self, image_dir):
        ...
        """ 
            ...
            float32 -> float64, input_image -> 1 dim vector 
        """
